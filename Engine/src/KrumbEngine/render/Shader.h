...
namespace KrumbEngine
{
	...
	private:
		...
		GLuint compileShader(const char* shaderSource, GLenum shaderType)
		{
			...
			switch (shaderType)
			{
				...
				default:
					...
					KRUMB_CORE_CRITICAL("Invalid shader type \"{}\" given when compiling shader.", shaderType);
					return -1;
			}
			...
			if (shaderID == 0)
			{
				KRUMB_CORE_CRITICAL("Error creating shader. glCreateShader({}) returned 0.", shaderTypeString); // TODO: Handle this better?
				return -1;
			}
			...
			{
				KRUMB_CORE_CRITICAL("Shader copilation failed. Type: {} - Error: {}", shaderTypeString, getInfoLog(ShaderObjectType::SHADER, shaderID));
				return -1;
			}
		...
		void init(const char* vertexShaderSource, const char* fragmentShaderSource)
		{
			...
			{
				KRUMB_CORE_CRITICAL("Shader link failed. Error: {}", getInfoLog(ShaderObjectType::PROGRAM, _programID));
				return;
			}
			...
			{
				KRUMB_CORE_CRITICAL("Shader validation failed. Error: {}", getInfoLog(ShaderObjectType::PROGRAM, _programID));
				return;
			}
	...
	public:
		...
		inline void initFromFile(const char* vertexShaderLoc, const char* fragmentShaderLoc)
		{
			...
			{
				KRUMB_CORE_CRITICAL("Could not open Vertex Shader File! Location: {}", vertexShaderLoc);
				return;
			}

			if (!fragmentFileStream.is_open())
			{
				KRUMB_CORE_CRITICAL("Could not open Fragment Shader File! Location: {}", fragmentShaderLoc);
				return;
			}
		...
		inline void use()
		{
			if (_init)
				...
				KRUMB_CORE_CRITICAL("Program must be initialized before being used.");
		}
		...
		GLuint attribute(const char* attribName)
		{
			...
			if (iter == _attributeMap.end())
			{
				...
				if (attribLookup == -1)
				{
					KRUMB_CORE_CRITICAL("Could not find shader attribute: {}", attribName);
					return -1;
				}
				else
				{
		...
		GLuint uniform(const char* uniformName)
		{
			...
			if (iter == _uniformMap.end())
			{
				...
				if (uniformLookup == -1)
				{
					KRUMB_CORE_CRITICAL("Could not find shader uniform: {}", uniformName);
					return -1;
				}
				else
				{
